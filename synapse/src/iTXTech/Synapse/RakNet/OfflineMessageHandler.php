		...
		switch($packet::$ID){
			...
			case OpenConnectionRequest1::$ID:
				...
				if($packet->protocol !== $serverProtocol){
					...
					$pk->serverId = $this->sessionManager->getId();
					$this->sessionManager->sendPacket($pk, $address);
					Logger::notice("Refused connection from $address due to incompatible RakNet protocol version (expected $serverProtocol, got $packet->protocol)");
				}else{
			...
			case OpenConnectionRequest2::$ID:
				...
				if($packet->serverAddress->port === $this->sessionManager->getPort() or !$this->sessionManager->portChecking){
					...
					Logger::debug("Not creating session for $address due to mismatched port, expected " . $this->sessionManager->getPort() . ", got " . $packet->serverAddress->port);
				}
